...
public class TournamentPlanViewPanel extends JLayeredPane {
    ...
    //Farben der Ebenen
    private static Color backFirst = Color.WHITE,  backSecound = Color.ORANGE; // Farben der Ebenen
    //eingeloggter User
    ...
    /**
     * Konstruktor 
     * Erstellt das Panel und seine inneren Panels, zeichnet es und
     * zeigt es an.
     *
     * @param frame Fenster in dem das Panel angezeigt wird.
     * @param t Das Turnier aus dem der Spiel erstellt werden soll
     * @param match Soll die Matchansicht angezeigt werden?
     * @param u aktuell eingeloggte User
     *
     * @pre Turnier wurde korrekt erstellt und ist mit Daten gefüllt
     */
    public TournamentPlanViewPanel(TournamentPlanView frame, TournamentPlan t, boolean match, User u) {
        ...
        while (lround.hasNext()) {
            ...
            while (lphase.hasNext()) {
                ...
                if (actPhase.getOutTransition() != null) {
                    createPhaseLabel(actPhase.getName(), actPhase.getOutTransition().getPauseMinutes());
                } //Sonstige Runden
                else {
                    createPhaseLabel(actPhase.getName(), actPhase.getInTransition().getPauseMinutes());
                }
                //Panels erstellen
                createPanel(match, actPhase.getGroups().listIterator(), phaseStat, actPhase.getOutTransition());
                //Phasenstatus speichern
                teamViewVisiblePhases.add(phaseStat);
                //Aktuelle Phase ist beendet oder letzte -> nächste nicht anzeigen bzw nicht editierbar
    ...
    /**
     * Erstellt ein Panel für die Darstellung einer Gruppe und setzt seinen Status
     *
     * @param ansicht Matchansicht anzeigen
     * @param Listiterator Gruppe die angezeigt werden soll
     * @param stat Aktivieren bzw anzeigen der Gruppe
     * @param Transition Übergang zwischen Phasen
     */
    private void createPanel(boolean ansicht, ListIterator<Group> lGroup, boolean stat, Transition proc) {
        ...
        while (lGroup.hasNext()) {
            ...
            //Matchansicht
            if (ansicht) {
                //Turnier ist abgeschlosen -> keine Änderung möglich
                if (actTourn.getTournament().getState() == TournamentState.finished) {
                    tmp.add(new MatchPanel(actGroup, actFrame, actTourn.getTournament().getFieldNames(), null, actTourn.getTournament().getReferees(), actTourn.getTournament().getLeaders(), stat));
                } //Turnier nicht abgeschlossen -> Änderungen je nach Benutzerrechten
                else {
                    tmp.add(new MatchPanel(actGroup, actFrame, actTourn.getTournament().getFieldNames(), actUser, actTourn.getTournament().getReferees(), actTourn.getTournament().getLeaders(), stat));
                }
            } //Teamansicht
            else {
                TeamPanel teamP;
                ...
                if (proc != null) {
                    teamP = new TeamPanel(actGroup, proc.getNumProceedantsPerGroup(), actFrame, true);
                } //Finale Phase -> nur der erste wird als weiter kommend markiert
                else {
    ...
    private void setPanelPositions(int width) {
        ...
        for (int i = 0; i < ViewLabels.size(); i++) {
            ...
            if ((Boolean) ViewLabels.get(i).get(0)) {
                ...
                int x = (width / 2) - ((Integer) ViewPanels.get(panelNum).get(ViewPanels.get(panelNum).size() - 1) / 2);
                //Iteration über die Gruppen der Phasen
                for (int j = 0; j < ViewPanels.get(panelNum).size() - 1; j++) {
                    //Position der Panels setzen
                    ((JPanel) ViewPanels.get(panelNum).get(j)).setLocation(x, y);
                    //Panel hinuzufügen
                    ...
                    x += ((JPanel) ViewPanels.get(panelNum).get(j)).getSize().width + GROUPSPACE;
                }
     ...
     *
     * @param Namen der Phase
     * @param Zeit die zwischen den Runden bzw Phasen pausiert wird
     */
    ...
    /**
     * Setzt die Position der Labels aus ViewLabels Panel und erzeugt Hintergründe für die Gruppen
     * und zeigt sie im Panel an
     */
              ...
              //Gruppenlabel
                if ((Boolean) ViewLabels.get(i).get(0)) {
                    ...
                    if (i > 0) {
                        ...
                        pause.setLocation(pause.getLocation().x, pause.getLocation().y + H_GROUPLABEL);
                        //Keine Linie zwischen Runden
                        line.setVisible(false);
                        gBeg = y;
                    }
                    //Größe,Position der Gruppenlabels setzen und hinzufügen
                    ((JLabel) ViewLabels.get(i).get(1)).setSize(getSize().width, H_GROUPLABEL);
                    ...
                    if (changelabel == 1) {
                        ((JLabel) ViewLabels.get(i - 1).get(1)).setVisible(false);
                        ((JLabel) ViewLabels.get(i - 1).get(2)).setVisible(false);
                        ((JLabel) ViewLabels.get(i - 2).get(1)).setText(((JLabel) ViewLabels.get(i - 1).get(1)).getText());
                    }
                    y += H_GROUPLABEL;
                    changelabel = 0;
                }
                //Phasenlabel
                    ...
                    int heigth = calcPhaseHeight(ViewPanels.get(i - groupLabels));
                    if (heigth < H_PHASE) {
                        heigth = H_PHASE;
                    }
                    //Position und Größe der Phasenlabels links und rechts setzen
                    ((JLabel) ViewLabels.get(i).get(1)).setSize(W_PHASELABEL, heigth);//Links
                    ((JLabel) ViewLabels.get(i).get(1)).setLocation(x, y);
                    ((JLabel) ViewLabels.get(i).get(2)).setSize(W_PHASELABEL, heigth);//Rechts
                    ((JLabel) ViewLabels.get(i).get(2)).setLocation(getSize().width - W_PHASELABEL, y);
                    ...
                    changelabel++;
                    //Linie zwischen Phasen erstellen, positionieren und hinzufügen
                    line = new JLabel();
                    line.setOpaque(true);
                    line.setBorder(BorderFactory.createLineBorder(Color.black));
                    line.setBounds(W_PHASELABEL, y, getSize().width - (2 * W_PHASELABEL), 1);
                    add(line);
                    moveToFront(line);
                    //Pause zwischen Abschnitten hinzufügen
                    pause = (JLabel) ViewLabels.get(i).get(3);
                    pause.setBounds(W_PHASELABEL, y, getSize().width - (2 * W_PHASELABEL), 20);
    ...
    /**
     * Berechnet die maximale Breite des Panels in Pixel für die Darstellung der Gruppen
     *
    ...
    private int calcMaxWidth() {
        ...
        for (int i = 0; i < ViewPanels.size(); i++) {
            ...
            for (int j = 0; j < ViewPanels.get(i).size(); j++) {
                tmp += ((JPanel) ViewPanels.get(i).get(j)).getSize().width + GROUPSPACE;
            }
            if (tmp > x) {
                x = tmp;
            }
     ...
     *
     * @param ArrayList der Gruppen in einer Runde
